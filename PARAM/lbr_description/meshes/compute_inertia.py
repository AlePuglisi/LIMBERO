...
density = 1.25  #[g/cm^3], Poly Lactic Acid

GRIEEL_links = ["gripper_Link_Hand", "gripper_Link_finger1", "gripper_Link_finger2", "gripper_Link_finger3"]
...
density = 1.21 # new density 

LIMBERO_links = ["base", "limb_root", "coxa", "femur", "tibia", "limb_end"]
GRIEEL_MODULE_links = ["wristH_Link", "wristV_Link", "driving_Link"]
volume = []
mass = []
CoM = []
inertia_matrix = []
...
for link in GRIEEL_MODULE_links:
    mesh_path = get_package_share_directory('lbr_description')
    stl_file = mesh_path + "/meshes/Grieel_gripper_mode/" + link + ".STL"
    #stl_CoM_file = mesh_path + "/meshes/CoM_centered/" + link + "_CoM.stl"
    mesh = trimesh.load(stl_file)
    #mesh_CoM = trimesh.load(stl_CoM_file)
    volume.append(mesh.volume)              # [m^2]
